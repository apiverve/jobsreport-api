...
class JobsreportValidationException implements Exception {
  final List<String> errors;

  JobsreportValidationException(this.errors);

  @override
  String toString() => 'JobsreportValidationException: ${errors.join("; ")}';
}
...
/// Jobs Report API client.
///
/// For more information, visit: https://apiverve.com/marketplace/jobsreport?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [date] - Optional date in YYYY-MM format for historical lookup. Omit for current data. [format: date]
class JobsreportClient {
  final String apiKey;
  ...
  JobsreportClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/jobsreport',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [JobsreportValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw JobsreportValidationException(errors);
    }
  }

  /// Execute a request to the Jobs Report API.
  ///
  /// Parameters are validated before sending the request.
  Future<JobsreportResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw JobsreportException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return JobsreportResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw JobsreportException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw JobsreportException('Resource not found');
      } else {
        throw JobsreportException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is JobsreportException) rethrow;
      throw JobsreportException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Jobs Report API client.
class JobsreportException implements Exception {
  final String message;

  JobsreportException(this.message);

  @override
  String toString() => 'JobsreportException: $message';
}
